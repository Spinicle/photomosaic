...
TILE_SIZE      = 16 	# Height/Width of tiles
TILE_MATCH_RES = 1		# Tile matching resolution
ENLARGEMENT    = 1		# Final image englargment

TILE_BLOCK_SIZE = TILE_SIZE / max(min(TILE_MATCH_RES, TILE_SIZE), 1)
WORKER_COUNT = max(cpu_count() - 1, 1)
OUT_FILE = 'unnamed-mosaic.jpeg'
EOQ_VALUE = None
...
class TileProcessor:
	...
	def __process_tile(self, tile_path):
		try:
			...
			large_tile_img = img.resize((TILE_SIZE, TILE_SIZE), Image.Resampling.LANCZOS)
			small_tile_img = img.resize((int(TILE_SIZE/TILE_BLOCK_SIZE), int(TILE_SIZE/TILE_BLOCK_SIZE)), Image.Resampling.LANCZOS)
...
class TargetImage:
	...
	def get_data(self):
		...
		# Crop image so we use whole number of tiles
		if w_diff or h_diff:
...
def fit_tiles(work_queue, result_queue, tiles_data):
	# This function is run by the worker processes, one on each CPU core
...
def get_filename(img_path):
	...
	filename_tuple = os.path.splitext(file)

	filename = filename_tuple[0] + "_mosaic.jpeg"
...
def build_mosaic(result_queue, all_tile_data_large, original_img_large, img_path):
	...
	OUT_FILE = get_filename(img_path)
